...
int minimax(int depth, bool isMaximizingPlayer, vector<vector<u_int8_t>>& board, u_int8_t currentPlayer, set<Pos>& perim) {
    // Check if the game has reached a terminal state, the desired depth, or if the evaluation is really bad
    // Return the evaluation value of the position if it is a terminal state or depth limit is reached
    int evaluation = eval(board, currentPlayer);
    if (depth == 0 || evaluation == 999999999)
    {
        return evaluation;
    }
    ...
    for (i = 0; i < tam; i++) {
        ...
        //Check if the new board configuration has already been explored. If not, add it to the knownBoards set
        uint64_t boardHash = calculateHash(board);
        bool isExplored = false;
        #pragma omp critical
        {
            if (knownBoards.count(boardHash) != 0) isExplored = true;
            knownBoards.insert(boardHash);
        }
